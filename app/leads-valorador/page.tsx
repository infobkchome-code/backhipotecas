...
export default function LeadsValoradorPage() {
  ...
  async function load() {
    ...
    try {
      ...
      sp.set("limit", "100");
      const r = await fetch(`/api/leads-valorador?${sp.toString()}`, { cache: "no-store" });
      const j = await r.json();
      if (!j?.ok) throw new Error(j?.error || "Error cargando leads");
      setRows(j.data || []);
    } catch (e: any) {
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.map((r) => {
              ...
              const wa = r.phone
                ? `https://wa.me/${String(r.phone).replace(/\D/g, "")}`
                : null;
